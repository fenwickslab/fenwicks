...
def get_tpu_estimator(n_trn, n_val, model_func, model_dir, ws_dir=None, ws_vars=None, trn_bs=128, val_bs=None):
    ...
    return tf.contrib.tpu.TPUEstimator(model_fn=model_func, model_dir=model_dir, train_batch_size=trn_bs,
...
def get_clf_model_func(model_arch, opt_func, reduction=tf.losses.Reduction.MEAN):
    def model_func(features, labels, mode, params):
        ...
        return tf.contrib.tpu.TPUEstimatorSpec(mode=mode, loss=loss, train_op=train_op, eval_metrics=tpu_metrics)
